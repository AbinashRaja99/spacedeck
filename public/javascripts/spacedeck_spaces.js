...
var SpacedeckSpaces = {
  ...
  data: {
    ...
    winner:{},
    comments:{},
    active_comment_userid:'',
    timeIntervalId:0,
    active_space_vote:false,
    is_voted_space:false
  },

  methods: {
    active_space_voting: function() {
      updatdeVotestatus((d) =>{
        this.active_space_vote=d.value_status;
      })
    },
    UVC:function(){
        updatdeVoteCount(()=>{
            check_spaVote((s)=>{
                this.is_voted_space=s.check;
            });
        });
    },
    reset_my_vote:function()
    {
      resetVote(()=>{
           check_spaVote((s)=>{
                this.is_voted_space=s.check;
    ...
    guest_logout: function() {
      if ("local;Storage" in window && localStorage) {
          ...
          if (space.space_type == "folder") {
            ...
            load_spaces(space_id, is_home, function(spaces) {
              ...
              load_history(space, function(history) {
                ...
                try{
                  localStorage.setItem("parentSpaceId",space._id);
                }catch(e){}
                getVoteStatus((d)=>{
                  this.active_space_vote=d.voting_status
                });
                check_spaVote((s)=>{
                  this.is_voted_space=s.check;
                });
    ...
    activate_access_vote: function() {
        votingResult((w)=>{
          this.winner=w;
        });
      this.activate_modal('result');
    },
    activate_comments: function() {
      ...
      this.activate_modal('comment');
    },
    refresh_comments:function()
    {
      console.log("refresh_comments");
      // console.log(localStorage.getItem("activeuserId"))
      this.active_comment_userid=localStorage.getItem("activeuserId");
      this.timeIntervalId=setInterval(this.activate_comments, 1000);
    },
    close_comments: function() {
      clearInterval(this.timeIntervalId);
      this.close_modal();
    },
    delete_comments: function(min) {
      delete_comment(min);
    },
    like_comments: function(min) {
      like_comment(min);
    },
    dislike_comments: function(min) {
      dislike_comment(min);
    },
